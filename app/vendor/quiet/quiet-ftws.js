...
var Quiet = (function() {
    ...
    // FTWS file-only patch: allow profiles to be embedded before quiet.js loads.
    // This avoids XMLHttpRequest/fetch from file://, which many browsers block.
    if (typeof window !== "undefined" && window.FTWS_EMBEDDED_PROFILES_JSON) {
        profiles = window.FTWS_EMBEDDED_PROFILES_JSON;
    ...
    function onEmscriptenLocateFile(path, prefix) {
        if (path.endsWith(".mem")) {
            if (typeof window !== "undefined" && window.FTWS_EMBEDDED_MEM_URI) {
                return window.FTWS_EMBEDDED_MEM_URI;
            }
            if (typeof memoryInitializerPrefixURL !== "undefined") {
    ...
    function transmitter(opts) {
        ...
        // FTWS safety patch: on some browser/profile combinations clamp_frame_len may
        // return 0. The original transmit() loop then slices 0 bytes forever and the
        // page appears to freeze immediately after clicking send. Always force a
        // positive frame length and prefer the profile's declared frame_length.
        if (!isFinite(frame_len) || frame_len < 1) {
            var fallbackFrameLen = profileObj && Number(profileObj.frame_length);
            frame_len = (isFinite(fallbackFrameLen) && fallbackFrameLen > 0) ? Math.floor(fallbackFrameLen) : 1;
        }
        frame_len = Math.max(1, Math.floor(frame_len));
        ...
        var transmit = function(buf) {
            ...
            if (!buf || typeof buf.byteLength !== 'number') {
                throw new Error('quiet transmitter expected an ArrayBuffer');
            }
            if (!isFinite(frame_len) || frame_len < 1) {
                frame_len = Math.max(1, Math.floor((profileObj && Number(profileObj.frame_length)) || 1));
            }
            // slice up into frames and push the frames to a list. Guard against
            // zero-length slices so a bad profile can never lock the browser.
            for (var i = 0; i < buf.byteLength; ) {
                var end = Math.min(i + frame_len, buf.byteLength);
                if (end <= i) {
                    throw new Error('quiet transmitter frameLength is invalid: ' + frame_len);
                }
                var frame = buf.slice(i, end);
                i += frame.byteLength;
                payload.push(frame);
            }
    ...
    function gUMConstraints() {
        // FTWS v6 patch:
        // Chrome/Android rejects constraints that mix legacy optional/mandatory with
        // modern specific constraints like deviceId/echoCancellation.
        // Always prefer clean standard constraints. Only fall back to legacy optional
        // constraints on very old browsers and only when no deviceId is requested.
        var requestedDeviceId = (typeof window !== "undefined" && window.FTWS_AUDIO_DEVICE_ID) ? window.FTWS_AUDIO_DEVICE_ID : "";

        function standardAudioConstraints() {
            // FTWS GMSK patch: line-app normally pre-opens one shared stream.
            // If Quiet must open its own stream, mirror the exact route-safe/raw
            // constraints selected by line-app instead of silently changing routes.
            var supplied = (typeof window !== "undefined" && window.FTWS_AUDIO_CONSTRAINTS) ? window.FTWS_AUDIO_CONSTRAINTS : null;
            var audio = supplied ? Object.assign({}, supplied) : {
        ...
        // Modern Chrome/Edge/Android WebView path. Do NOT include optional/mandatory.
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            return standardAudioConstraints();
        }
        ...
        if (navigator.mozGetUserMedia !== undefined) {
            ...
            if (requestedDeviceId) mozAudio.deviceId = { exact: requestedDeviceId };
            return { audio: mozAudio };
        }

        // Very old WebKit fallback. Avoid sourceId when it is not needed.
        if (navigator.webkitGetUserMedia !== undefined) {
            if (requestedDeviceId) {
                // Newer WebKit-compatible engines should accept standard deviceId.
                return standardAudioConstraints();
            }
            return {
                audio: {
    ...
    function createAudioInput() {
        ...
        window.setTimeout(function() {
            var existing = (typeof window !== "undefined") ? (window.FTWS_AUDIO_STREAM || window.FTWS_AUDIO_STREAM) : null;
            if (existing && existing.getAudioTracks && existing.getAudioTracks().length) {
                try {
                    audioInput = audioCtx.createMediaStreamSource(existing);
                    window.quiet_receiver_anti_gc = audioInput;
                    audioInputReady();
                    return;
                } catch (err) {
                    // fall through to a fresh getUserMedia
                }
            }
    ...
    function receiver(opts) {
        ...
        if (gUM === undefined) {
            if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
                gUM = function (constraints, success, fail) {
        ...
        var shared = (typeof window !== "undefined") ? (window.FTWS_AUDIO_STREAM || window.FTWS_AUDIO_STREAM) : null;
        var hasShared = !!(shared && shared.getAudioTracks && shared.getAudioTracks().length);

        if (gUM === undefined && !hasShared) {
        ...
        var scriptProcessor = audioCtx.createScriptProcessor(sampleBufferSize, 1, 1);
